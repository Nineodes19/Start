...
def insert():
  ...
  # 将输入的学生信息保存到字典
    ...
    print("\033[1;34;47m宋词信息录入完毕!!!\033[0m")
...
def show_poems(poemList):
    ...
    format_titleID = "\033[1;31;47m---{:^6}\033[0m"
    #print(format_title.format("ID", "宋词名", "词人", "内容", "思想"))
    format_data = "\t{:^6}"
    for info in poemList:
        print(format_titleID.format("ID"))
...
def delete():
    mark = True # 标记是否循环
    while mark:
        poemId = input("\033[1;34;47m>请输入要删除的宋词ID：\033[0m")
        if poemId !="": # 判断要删除的学生ID是否存在
            if os.path.exists(filename):
                ...
                poem_old = []
                ifdel = False # 标记是否删除
            if poem_old: # 如果存在宋词信息
                choose = input("\033[1;34;47m>请确认是否删除该信息？（y/n）\033[0m")
                if choose == "y":
                    with open(filename, 'w+') as wfile:
                        ...
                        for list in poem_old:
                            ...
                            if d['id'] != poemId:
                                ...
                                ifdel = True # 标记已经删除
                        if ifdel:
                            print("\033[1;34;47mID为%s的宋词信息已经被删除...\033[0m" % poemId)
                        else:
                            print("\033[1;34;47m没有找到ID为%s的宋词信息...\033[0m" % poemId)
                else:
                    print("\033[1;31;47m\t操作者取消删除，删除失败！\033[0m")

            else:
                 print("\033[1;34;47m不存在宋词信息")
                 break
            #show() # 显示全部宋词信息
...
def modify():
    ...
    if os.path.exists(filename):
        ...
        return
    poemid = input("\033[1;34;47m>请输入要修改的宋词ID：\033[0m")
    with open(filename, 'w') as wfile:
        for item in poem_Id:
            d = dict(eval(item))
            if d['id'] == poemid:
                ...
                while True: # 输入要修改的信息
                    try:
                        ...
                        d["content"] =input("\033[1;34;47m>请输入宋词的内容：\033[0m")
                        d['idea'] = input("\033[1;34;47m>请输入宋词的思想：\033[0m")
                    except:
                        print("\033[1;34;47m您输入有误，请重新输入！\033[0m")
                    else:
                        break
        ...
        mark = input("\033[1;34;47m是否继续修改其他宋词信息？(y/n):\033[0m")
        if mark == "y":
            modify()
